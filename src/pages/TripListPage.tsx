            ...
            <div className="bg-white px-6 pt-10 pb-6 shadow-sm sticky top-0 z-10 border-b border-gray-50">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                        ...
                        {/* Emergency Restore Button for Trip 29 - Owner Only */}
                        {user?.id && (
                            <button
                                onClick={async () => {
                                    const toastId = toast.loading('Running Deep Discovery...');
                                    try {
                                        // 0. Verify Ownership (Strict DB Check)
                                        const { data: trip } = await supabase.from('trips').select('user_id').eq('id', 29).single();
                                        if (!trip || trip.user_id !== user.id) {
                                            toast.error('Only the trip owner can run recovery.', { id: toastId });
                                            // Fallback for UI if it was showing
                                            return;
                                        }
                                        ...
                                        const currentCompanions = config?.companions || [];
                                        const currentFlightInfo = config?.flight_info || {};
                                        ...
                                        // 3. Recover Companions
                                        const recoveredCompanions = new Set<string>(currentCompanions);
                                        // Add Payers
                                        ...
                                        // Update Config with Recovered Companions (Visual List)
                                        if (finalCompanions.length > currentCompanions.length) {
                                            await supabase.from('trip_config').upsert({
                                                trip_id: 29,
                                                companions: finalCompanions,
                                                flight_info: currentFlightInfo // Preserve existing or default
                                            }, { onConflict: 'trip_id' }); // Upsert to be safe if config missing

                                            // Also just update if it existed, simply:
                                            // await supabase.from('trip_config').update({ companions: finalCompanions }).eq('trip_id', 29);
                                            // But upsert is safer if config was purely missing.

                                            toast.success(`Recovered ${finalCompanions.length} companions!`, { id: toastId });
                                        }

                                        // 3b. FORCE RESTORE OWNER to trip_members (Real DB)
                                        // This fixes the "Owner missing from expenses" bug because Expense creation checks real membership or config?
                                        // Actually ExpensePage uses `trip_config.companions` mostly, but Trip List uses `trip_members`.
                                        // Restoring `trip_members` ensures the trip shows up for the user.
                                        const { error: memberError } = await supabase.from('trip_members').insert({
                                            trip_id: 29,
                                            user_id: user.id
                                        });
                                        if (memberError && memberError.code !== '23505') {
                                            console.error('Member restore error:', memberError);
                                        } else {
                                            console.log('Owner membership restored.');
                                        }
                                        ...
                                        if (itineraries && itineraries.length > 0) {
                                            ...
                                            toast.warning('No itineraries found, but membership restored.', { id: toastId });
                                        }
                                ...
                                // Mount check to hide from non-owners visuals
                                ref={async (btn) => {
                                    if (btn && user) {
                                        const { data } = await supabase.from('trips').select('user_id').eq('id', 29).single();
                                        // If data is null (trip not visible/found) or user mismatch, hide
