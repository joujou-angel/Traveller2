...
*   **功能**：
    ...
    *   **權限標示**：列表卡片以圖示區分權限：
        *   👑 **Owner** (黃色皇冠)：主辦行程，擁有完整權限。
        *   👥 **Shared** (藍色人員)：受邀行程，僅檢視權限。
...
*   **保護機制**：
    *   **RLS (Row Level Security)**：資料庫層級保全。Owner 可讀寫刪 (`FOR ALL`)，Member 僅可讀 (`FOR SELECT`)。
    *   **Route Protection**：未登入強制重導至 `/login`。

### 5.4 介面優化 (UI Refinements)
*   **圖示化按鈕**：全面導入 Lucide Icons (Check, X, Trash) 取代純文字按鈕。
